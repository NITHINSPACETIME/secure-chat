...
export const setUserDataAtom = atom(
  null,
  async (get, set, data: Partial<User> & { id: string; name: string }) => {
    ...
    if (newUser.statusMessage)
      localStorage.setItem("userStatusMessage", newUser.statusMessage);
    ...
    localStorage.setItem(
      "blockedUsers",
      JSON.stringify(newUser.blockedUsers || [])
    );
  ...
  async (get, set, updates: Partial<User>) => {
    ...
    if (updates.hasOwnProperty("statusMessage")) {
      if (updates.statusMessage)
        localStorage.setItem("userStatusMessage", updates.statusMessage);
      else localStorage.removeItem("userStatusMessage");
    }
    ...
    if (updates.blockedUsers) {
      localStorage.setItem(
        "blockedUsers",
        JSON.stringify(updates.blockedUsers)
      );
    }
