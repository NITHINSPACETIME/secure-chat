...
import { compressImage } from "../utils/services";
...
export function MessageInput({ conversationId }: MessageInputProps) {
  ...
  const [emojiCategory, setEmojiCategory] = useState<keyof typeof EMOJI_CATEGORIES>("Faces");
  ...
  const handleSend = async () => {
    ...
    if (attachment) {
        try {
            ...
            contentToSend = `[image]${compressed}|${contentToSend}`;
        } catch (e) {}
    }

    const encryptedContent = encryptMessage(contentToSend, mySecretKey, recipientKey);
    ...
    <div className="border-t border-border p-4 bg-background relative z-20">
          ...
          {replyTo && (
              ...
              >
                  ...
                  <div className="flex-1 overflow-hidden">
                      <p className="text-xs font-bold text-primary">Replying to message</p>
                      <p className="text-xs text-muted-foreground truncate">{replyTo.content.startsWith('[image]') ? '📷 Image' : replyTo.content}</p>
                  </div>
                  <button onClick={() => setReplyTo(null)} className="p-1 hover:bg-background rounded-full"><X className="w-4 h-4"/></button>
              </motion.div>
          )}
      </AnimatePresence>

      {/* Emoji Picker - Adjusted Position */}
      <AnimatePresence>
        {showEmojiPicker && (
            <motion.div 
                ...
                <div className="flex gap-1 p-2 bg-muted/50 overflow-x-auto no-scrollbar">
                    {Object.keys(EMOJI_CATEGORIES).map(cat => (
                        <button 
                            ...
                            className={`text-[10px] px-2 py-1 rounded-full transition-colors ${emojiCategory === cat ? 'bg-primary text-primary-foreground' : 'hover:bg-background text-muted-foreground'}`}
                        >
                            {cat}
                        </button>
                    ))}
      ...
      {/* Attachment Preview */}
       <AnimatePresence>
          {attachment && (
              ...
              >
                  <img src={attachment.url} alt="Preview" className="h-20 w-auto rounded-lg border border-border object-cover" />
                  <button 
      ...
      {isBlocked ? (
          <div className="p-3 bg-destructive/10 border border-destructive/20 rounded-xl text-destructive text-center text-sm font-medium flex items-center justify-center gap-2">
              <AlertTriangle className="w-4 h-4" /> You blocked this user. Unblock to send messages.
          </div>
      ) : (
        <div className="flex items-end gap-2">
            <input 
                ...
                className="hidden" 
                accept="image/*"
                onChange={(e) => e.target.files?.[0] && setAttachment({ url: URL.createObjectURL(e.target.files[0]), file: e.target.files[0] })}
            />
            ...
            <div className="flex-1 relative">
                <Textarea
                    ...
                    placeholder={status === "ready" ? "Type a secure message..." : statusMessage}
                ...
                <Button 
                    variant="ghost" 
                    size="icon" 
                    className={`absolute right-1 bottom-1 h-8 w-8 rounded-full ${showEmojiPicker ? 'text-primary' : 'text-muted-foreground'}`}
            ...
            <Button 
                onClick={handleSend} 
                disabled={status !== "ready" || (!message.trim() && !attachment) || isSending}
                size="icon" 
